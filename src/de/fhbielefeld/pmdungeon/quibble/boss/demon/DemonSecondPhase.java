...
public class DemonSecondPhase extends BossPhase
{
	...
	public DemonSecondPhase(BossBattle battle)
	{
		...
		{
			var face = i * 90;
			var proj = new ProjectileSpawner(40, new CreatureStats(), new Vector2((float)Math.pow(-1, i + 1) * -2, (float)Math.pow(-1, i + 1) * -2), bullet,
				battle.getBoss());
			var proj2 = new ProjectileSpawner(40, new CreatureStats(), new Vector2((float)Math.pow(-1, i + 1) * -2, (float)Math.pow(-1, i + 1) * -2), fireball,
				battle.getBoss());
			proj2.addPattern(new SpinMovementPattern(proj2, 40));
			proj.addPattern(new SpinMovementPattern(proj, 40));
			proj.currentBulletSpeed = 0.06f;
		...
		{
			var face = i * 45;
			var proj = new ProjectileSpawner(20, new CreatureStats(), new Vector2((float)Math.pow(-1, i + 1) * 4, (float)Math.pow(-1, i + 1) * -4), bullet,
				battle.getBoss());
